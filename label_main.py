...
from labels import B1_label
from labels import B1_outterbox_label
from labels import B73_label
from labels.utilities import check_caps_lock
class LabelPrinter:
    ...
    def print_label(self, label_type: str, parameters: dict, is_test=False):
        ...
        label_print_functions = dict(lision_label=lision_label.modify_and_print_lision_label,
                                     ...
                                     B1_label=B1_label.modify_and_print_B1_label,
                                     B1_outterbox_label=B1_outterbox_label.modify_and_print_B1_outterbox_label,
                                     B73_label=B73_label.modify_and_print_B73_label
                                     )
    ...
    def preprocess_data(self):
        ...
        # Iterate through the rows of the param_table
        for label_type, row in self.param_table.iterrows():
            if label_type != 'param_table':
                label_address = row['label_address']

                # Check if label_address is not NaN and corresponding data frame is not empty
                if not pd.isna(label_address) and not self.data[label_type].empty:
